  ...
  switch (type) {
    case ParamType.temperature:
      for (int x = 0; x < data.weatherList.length; x++) {
        result.add(
          FlSpot(i++,
              ((data.weatherList[x].tempRaw * mod).round().toDouble() / mod)),
        );
      }
      break;
    case ParamType.humidity:
      for (int x = 0; x < data.weatherList.length; x++) {
        result.add(
          FlSpot(i++,
              ((data.weatherList[x].humRaw * mod).round().toDouble() / mod)),
        );
      }
      break;
    case ParamType.pressure:
      for (int x = 0; x < data.weatherList.length; x++) {
        result.add(
          FlSpot(i++,
              ((data.weatherList[x].presRaw * mod).round().toDouble() / mod)),
        );
      }
      break;
    case ParamType.light:
      for (int x = 0; x < data.weatherList.length; x++) {
        result.add(
          FlSpot(
              i++, ((data.weatherList[x].lux * mod).round().toDouble() / mod)),
        );
      }
      break;
  }
